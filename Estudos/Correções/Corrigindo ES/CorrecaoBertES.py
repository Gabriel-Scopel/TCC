...
def normalize_grade(similarity, max_score=5.0):
    """Converte a similaridade em uma nota proporcional ao escore máximo."""
    return round(similarity * max_score, 1)  # Normaliza para escala de 0 a 5

# Carregar arquivo JSON com as respostas
with open("esData.json", "r", encoding="utf-8") as file:
    data = json.load(file)
...
with open("graded_responses_es.json", "w", encoding="utf-8") as output_file:
    json.dump(data, output_file, indent=4, ensure_ascii=False)

print("Correção concluída! As respostas foram salvas em 'graded_responses_es.json'")
