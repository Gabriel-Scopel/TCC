...
output_path = "D:\Downloads\ProjTCCMurilo\correcao_elmo_PT.json"
...
def obter_embedding(textos):
    """Gera embeddings ELMo para os textos fornecidos."""
    embeddings = model.signatures["default"](tf.constant(textos))
...
def calcular_similaridade(resposta_aluno, respostas_referencia):
    ...
    aluno_vec = obter_embedding([resposta_aluno])[0].reshape(1, -1)
    referencias_vec = np.array([obter_embedding([resp])[0] for resp in respostas_referencia])
...
# Processar cada questão e resposta com barra de progresso
for item in tqdm(data, desc="Processando questões"):
